...
public class IntroducerServiceTests
{
    ...
    {
        // Arrange
        _configManagerMock.Setup(r => r.GetDeviceAsync("introducer-1"))
    ...
    public async Task ProcessIntroductionAsync_AddsNewDevice()
    {
        ...
        _configManagerMock.Setup(r => r.GetDeviceAsync("introducer-1")).ReturnsAsync(introducer);
        _configManagerMock.Setup(r => r.GetDeviceAsync("new-device")).ReturnsAsync((SyncDevice?)null);
        ...
        _configManagerMock.Verify(r => r.UpsertDeviceAsync(It.Is<SyncDevice>(d => d.DeviceId == "new-device")), Times.Once);
    }
    ...
    public async Task ProcessIntroductionAsync_UpdatesExistingDevice()
    {
        ...
        _configManagerMock.Setup(r => r.GetDeviceAsync("introducer-1")).ReturnsAsync(introducer);
        _configManagerMock.Setup(r => r.GetDeviceAsync("existing-device")).ReturnsAsync(existingDevice);
        _configManagerMock.Setup(r => r.UpsertDeviceAsync(It.IsAny<SyncDevice>())).Returns(Task.CompletedTask);
    ...
    public async Task SetIntroducerAsync_UpdatesDeviceProperty()
    {
        ...
        // Assert
        _configManagerMock.Verify(r => r.UpsertDeviceAsync(It.Is<SyncDevice>(d => d.Introducer)), Times.Once);
    }
