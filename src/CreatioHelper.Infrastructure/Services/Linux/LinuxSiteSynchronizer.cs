...
public class LinuxSiteSynchronizer : ISiteSynchronizer
{
    ...
    private async Task StopServiceAsync(ServerInfo server, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await RunSshCommandAsync(server, $"sudo systemctl stop {server.ServiceName}", cancellationToken)
                .ConfigureAwait(false);
    ...
    private async Task StartServiceAsync(ServerInfo server, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await RunSshCommandAsync(server, $"sudo systemctl start {server.ServiceName}", cancellationToken)
                .ConfigureAwait(false);
