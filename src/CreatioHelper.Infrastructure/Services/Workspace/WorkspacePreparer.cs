...
public class WorkspacePreparer : IWorkspacePreparer
{
    ...
    public async Task PrepareAsync(string sitePath, CancellationToken cancellationToken = default)
    {
        var (_, quartzIsActiveOriginal) = await PrepareInternalAsync(sitePath, cancellationToken).ConfigureAwait(false);
    }

    public void Prepare(string sitePath, out bool quartzIsActiveOriginal)
    {
        (_, quartzIsActiveOriginal) = PrepareInternalAsync(sitePath).GetAwaiter().GetResult();
    }

    private async Task<(bool success, bool quartzIsActiveOriginal)> PrepareInternalAsync(string sitePath, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(sitePath) || !Directory.Exists(sitePath))
        {
            ...
            return (false, quartzIsActiveOriginal);
        }
    ...
    private async Task ExecuteModifiedBatchIfNeededAsync(string sitePath, string batchFileName, string targetFileRelativePath, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrEmpty(sitePath)) throw new ArgumentNullException(nameof(sitePath));
        ...
        string originalBatchContent = await File.ReadAllTextAsync(batchFilePath, cancellationToken).ConfigureAwait(false);
        string modifiedBatchContent = originalBatchContent.Replace("pause", "REM pause");
        await File.WriteAllTextAsync(batchFilePath, modifiedBatchContent, cancellationToken).ConfigureAwait(false);
        ...
        {
            await File.WriteAllTextAsync(batchFilePath, originalBatchContent, cancellationToken).ConfigureAwait(false);
        }
    }

    private void ExecuteModifiedBatchIfNeeded(string sitePath, string batchFileName, string targetFileRelativePath)
    {
        ExecuteModifiedBatchIfNeededAsync(sitePath, batchFileName, targetFileRelativePath).GetAwaiter().GetResult();
    }
